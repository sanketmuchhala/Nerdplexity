...
describe('costStatus', () => {
  ...
  it('respects the user’s billing statement for the account', () => {
    expect(costStatus(connection({ kind: 'gemini', name: 'Gemini', billing: 'no-billing' }), model('gemini-2.5-flash'))).toMatchObject({ cls: 'no-billing', free: true });
