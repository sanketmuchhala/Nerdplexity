...
function Pill<T extends string>({ value, options, displayMap, onChange, disabled }: {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        style={{ background: 'var(--s2)', border: '1px solid var(--b-hi)', color: 'var(--t2)', cursor: canOpen ? 'pointer' : 'default' }}
        onMouseEnter={e => { if (canOpen) (e.currentTarget as HTMLElement).style.borderColor = 'rgba(59,130,246,.3)'; }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--b-hi)'; }}
      >
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1.5 z-50 min-w-[160px] rounded-xl overflow-hidden animate-fade-in"
          style={{ background: 'var(--s1)', border: '1px solid var(--b-hi)', boxShadow: '0 8px 32px rgba(0,0,0,.8)' }}>
          {options.map(opt => {
            const active = opt === value;
            return (
              <button key={opt} onClick={() => { onChange(opt); setOpen(false); }}
                className="w-full flex items-center justify-between gap-3 px-3 py-2 text-xs text-left transition-colors"
                style={{ color: active ? 'var(--blue-bright)' : 'var(--t2)' }}
...
export function ChatHeader({ conversation, onOpenSettings, onProviderChange, onModelChange, availableProviders = [] }: Props) {
  const settingsBtn = (
  ...
  return (
    <div className="flex items-center gap-3 h-12 px-5 flex-shrink-0" style={{ borderBottom: '1px solid var(--b)' }}>
      <h1 className="text-sm font-medium truncate flex-1 min-w-0" style={{ color: 'var(--t2)' }}>
        {sanitizeDisplayText(conversation.title)}
      </h1>
      <div className="flex items-center gap-1.5 flex-shrink-0">
        <Pill value={conversation.provider} options={availableProviders as Provider[]}
          displayMap={PROVIDER_NAMES as Record<string,string>}
          onChange={p => onProviderChange?.(p as Provider)} disabled={availableProviders.length <= 1} />
        <Pill value={conversation.model} options={models}
          onChange={m => onModelChange?.(m)} disabled={models.length <= 1} />
      </div>
      {settingsBtn}
    </div>
  );
