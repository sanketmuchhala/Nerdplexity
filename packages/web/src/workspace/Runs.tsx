...
export function Runs({ openConversation }: { openConversation: (id: string) => void }) {
  ...
  return <div className="np-page">
    ...
    <div className="np-run-list">{runs.map(run => <button key={run.id} className={`np-run-row ${selected?.id === run.id ? 'selected' : ''}`} onClick={() => setSelected(selected?.id === run.id ? null : run)}>
      ...
      <div><strong>{run.prompt}</strong><span>{run.model} · {run.mode === 'agent' ? 'Document agent' : 'Chat'} · {new Date(run.startedAt).toLocaleString()}</span></div><span className="np-run-duration">{(run.durationMs / 1000).toFixed(1)}s</span><span className={`np-label ${run.status === 'failed' || run.status === 'interrupted' ? 'error' : ''}`}>{STATUS_LABEL[run.status] ?? run.status}</span>
    </button>)}</div>
