...
import { exportText, runtimeName } from './api';
import { useRun } from './useRun';

export function ChatWorkspace({ run, documents, onModels, onDocuments, onSettings }: { run: ReturnType<typeof useRun>; documents: WorkspaceDocument[]; onModels: () => void; onDocuments: () => void; onSettings: () => void }) {
  const { activeConversation, settings, saveSettings, getCurrentProvider, getApiKey } = useChat();
  const conversation = activeConversation();
  const provider = conversation?.provider || getCurrentProvider();
  const local = provider === 'local-ollama';
  const runtime = conversation?.runtime || settings?.localRuntime || 'ollama';
  const model = conversation?.model || (local ? settings?.localModels?.[runtime] : undefined);
  ...
  const ready = !!model && (local || !!getApiKey(provider));
  ...
  return <div className="np-chat">
    ...
    <div className={`np-chat-scroll ${empty ? 'empty' : ''}`} ref={scroll} onScroll={() => { const el = scroll.current; if (el) { sticky.current = el.scrollHeight - el.scrollTop - el.clientHeight < 90; setShowScroll(!sticky.current); } }}>
      {empty ? <div className="np-welcome">
        ...
        {!ready && <button className="np-connect-prompt" onClick={local ? onModels : onSettings}>{local ? 'Connect a local model to get started' : 'Add your API key to get started'}<ArrowRight size={14}/></button>}
      </div> : <div className="np-thread">
        {messages.map(message => <Message key={message.id} message={{ ...message, timestamp: message.createdAt }}/ >)}
    ...
    <div className="np-composer-area">
      ...
      <div className="np-composer-footnote"><span>{agent ? `Agent access: all ${documents.length} workspace documents · search and read only` : local ? 'Local model requests stay on this machine.' : `Prompts are sent to ${PROVIDER_NAMES[provider]} using your API key.`}</span><span>Shift + Enter for a new line</span></div>
    </div>
  </div>;
}
