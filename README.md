...
Click **Open PDF** on a chat attachment to view its pages, change pages, zoom, or download the original. Chat saves original PDFs with your thread so previews survive reloads. Older attachments saved only text: use **Choose original PDF** once to restore their preview. Thread and account exports include saved PDF originals; thread imports accept up to 50 MB. Workspace imports save extracted text only.
