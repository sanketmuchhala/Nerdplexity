...
Models are labelled **On this machine** (no hosted fee), **Free model** (listed at $0), **Free-tier account**, a catalog price, or **Price unknown**. For Groq, Cerebras, Gemini, Mistral, and SambaNova, a connection marked **No billing enabled** contributes its available models to the Free Router. The provider still controls model access and quotas, and the server rediscovers the selected model before every call. Other hosted providers require model-level $0 pricing.
