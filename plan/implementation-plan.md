...
- P11.5 Free Agent always multi-model, with a live panel (2026-09-15), branch `claude/free-router`. Owner direction: the Free Agent must use several models for every answer, not one like the Free Router, and show which models are used, what each does, their thinking, and what passes between them, in real time. Automatic now answers a simple message with one draft checked by a second model (2 requests); one model alone only with tools, a single usable model, or Quick. Drafters, specialists, and the planner stream their text and reasoning as `agent_output` events (batched every 150 ms) and keep the reasoning on the finished step. The panel opens while the agent works: a flow line (plan → drafts or parts → checks and writes), and a card per step with the model's logo, ID, connection, why it was chosen, time, live thinking and output, and the hand-offs; it folds to one line when done. The answer stays credited to Nerdplexity; the earlier choice to hide model names in the chat is reversed for the panels (Free Agent and Free Router). The status line names the models at work. Found while testing: models on this machine take turns through the local queue, so their drafts stream one after another. Deep Research plan revised after the Gemini research (`plan/deep-research.md`, `plan/research/deep-research.md` with review notes: no sources given, several limits outdated).
- Deep Research R1 (2026-09-15), branch `claude/free-router` (`backend/src/runtime/research.ts`, docs `backend/docs/deep-research.md`). Route strategy `research`, needing an Exa key: the planner lists perspectives and sub-questions with queries (STORM-style), Exa searches return page text (`exaPages`), sources are picked in turn per query without repeats, readers on different models extract facts with exact quotes and a quote is kept only if it is on the page (normalized matching, ellipses in order, 12-character minimum), an outline assigns notes to sections, the strongest model writes a report citing numbered sources from checked notes only, and a citation check by matching flags citations to no source. Budgets quick / standard / deep (3/6/10 searches, 6/12/20 sources); the app sends standard. The Free Agent's step runner is now `stepRunner`, shared by both. Web app: a per-thread **Deep research** switch in the composer (Free Agent only, needs an Exa key), research steps in the live panel, and the sources saved as numbered links in citation order. Found while testing: a message sent right after switching a tool on went out before the setting was saved, so it ran without the tool; the composer now sends after the pending save.
