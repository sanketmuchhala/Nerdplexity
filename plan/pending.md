...
- **Deep Research** (2026-09-15, R0 and R1 of `plan/deep-research.md`): one search round, quotes checked against pages, cited report, citation check by matching. Open work is now planned in detail in [`plan/deep-research-quality.md`](deep-research-quality.md) (2026-09-16): the pipeline is never told today's date, freshness is one blunt run-wide filter, Exa is used as a page-text pipe (deprecated parameters, no `maxAgeHours`, no `summary`, no `/contents`, no cost shown), and each stage drops what the next needs (sub-question coverage, outline note assignments, corroboration). Phases: 0 measure, A freshness and Exa, B reading quality, C synthesis correctness, D second round, E control, F evaluation. R5 (free-router) is unchanged and still after those.
- **P11.3, Free Agent** (2026-09-14, branch `claude/free-router`): multi-model assistant on the Free Router's pool, ranking, health, and fallback (`runtime/agent.ts`, docs `backend/docs/free-agent.md`). Always at least two models per answer, with a live panel of every model's thinking, output, and hand-offs (2026-09-15). Default model since 2026-09-14, with user settings for behavior, drafts, and the model for each role, and per-step attempt limits instead of a per-message cap. Open: Bench does not yet measure the agent end to end against the best single model; strategy is keyword-based; drafts are not streamed; no tools in ensemble or plan; settings are per user, not per thread.
