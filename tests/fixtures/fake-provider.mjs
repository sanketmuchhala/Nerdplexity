...
http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/_log') {
    ...
    res.writeHead(200, { 'content-type': 'application/json' }).end(JSON.stringify(prompt ? log.filter(e => e.prompt === prompt) : log));
    return;
  }
  ...
  if (req.method !== 'POST' || url.pathname !== '/v1/chat/completions') { res.writeHead(404).end(); return; }

  const body = await readJSON(req);
  const prompt = [...body.messages].reverse().find(m => m.role === 'user')?.content ?? '';
  const entry = { prompt, model: body.model, aborted: false, completed: false, tokens: 0 };
  log.push(entry);
  ...
  if (body.model === 'slow-model') {
    for (let i = 0; i < 30; i++) if (!(await send(delta(`token-${i} `), 100))) return;
    ...
    for (let i = 0; i < bytes.length; i += 3) { if (res.destroyed) return; res.write(bytes.subarray(i, i + 3)); }
  }
  res.write(frame({ choices: [{ delta: {}, finish_reason: 'stop' }] }));
  res.write(frame({ choices: [], usage: { prompt_tokens: 5, completion_tokens: entry.tokens } }));
  entry.completed = true;
  res.end('data: [DONE]\n\n');
